...
/**
 * Check if customer is a new user (has no username/password_hash)
 */
function isNewUser($email) {
    global $pdo;
    $stmt = $pdo->prepare("SELECT username, password_hash FROM customers WHERE email = ?");
    $stmt->execute([$email]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if (!$user) return true;
    return (empty($user['username']) || empty($user['password_hash']));
}

/**
 * Update customer basic info (username and password_hash)
 */
function updateCustomerProfile($email, $username, $password) {
    global $pdo;
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
    $stmt = $pdo->prepare("UPDATE customers SET username = ?, password_hash = ? WHERE email = ?");
...
function verifyCustomerLogin($username, $password) {
    global $pdo;
    $stmt = $pdo->prepare("SELECT id, password_hash, email FROM customers WHERE username = ? OR email = ?");
